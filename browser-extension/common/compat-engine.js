(() => {
  ...
  async function applyCompat() {
    ...
    applying = (async () => {
      const prefs = await local({ enabled:true, mode:"standard" });
      const enabled = await enabledRulesets();
      const active = enabled.includes(COMPAT_RULESET);
      const wanted = prefs.enabled !== false && prefs.mode !== "ultra";
      if (active !== wanted) {
        try {
      ...
      return { ok:true, active:finalActive, wanted };
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area === "local" && (changes.enabled || changes.mode)) applyCompat().catch(() => {});
  });
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg?.type === "getCompatStats") {
      Promise.all([local({enabled:true,mode:"standard"}), enabledRulesets()]).then(([prefs, enabled]) => {
        sendResponse({ ok:true, active:enabled.includes(COMPAT_RULESET), mode:prefs.mode === "ultra" ? "ultra" : "standard" });
