...
public class MainActivityV140 extends Activity {
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        try {
            ...
            renderAll();
            loadFullListAsync();
            SystemLogStore.info(this, "UI", "MainActivityV140 uruchomiona");
        } catch (Throwable t) {
    ...
    private void buildUi() {
        ...
        TextView title=tv("xADKILLER",29,text,true);title.setLetterSpacing(.07f);names.addView(title);names.addView(tv("DNS + SMART + ADAPTIVE AI • BY SWIR",11,blue,true));gap(root,12);
        ...
        Switch smartOn=toggle("Smart detection",p.getBoolean(SmartAdAccessibilityService.KEY_SMART_ENABLED,true),text);smartOn.setOnCheckedChangeListener((x,on)->{p.edit().putBoolean(SmartAdAccessibilityService.KEY_SMART_ENABLED,on).apply();renderSmart();});smart.addView(smartOn,wrap());
        ...
        LinearLayout stats=card(surface);root.addView(stats,wrap());stats.addView(section("STATYSTYKI DNS",muted));dnsStats=tv("0 zablokowanych • 0 zapytań",18,green,true);stats.addView(dnsStats);domainCount=tv("0 domen",12,text,false);stats.addView(domainCount);lastBlocked=tv("Ostatnia blokada: —",12,muted,false);stats.addView(lastBlocked);gap(root,9);
        ...
        TextView footer=tv("xADKiller 1.4.0 • Adaptive AI • BY SWIR",11,muted,false);footer.setGravity(Gravity.CENTER);root.addView(footer,wrap());
    ...
    private void renderVpn(boolean running,long queries,long blocked,int domains,String last) {
        ...
        if(running){vpnStatus.setText("AKTYWNA");vpnStatus.setTextColor(c(R.color.swir_green));vpnButton.setText("WYŁĄCZ OCHRONĘ");vpnButton.setBackground(round(c(R.color.swir_red),12));vpnButton.setTextColor(Color.WHITE);}
        else{vpnStatus.setText("WYŁĄCZONA");vpnStatus.setTextColor(c(R.color.swir_red));vpnButton.setText("WŁĄCZ OCHRONĘ");vpnButton.setBackground(round(c(R.color.swir_blue),12));vpnButton.setTextColor(Color.BLACK);}
        dnsStats.setText(blocked+" zablokowanych • "+queries+" zapytań");domainCount.setText(domains+" domen na liście");lastBlocked.setText("Ostatnia blokada: "+(last==null?"—":last));
    }
    ...
    private void renderPrivateDns(){if(privateDns==null)return;PrivateDnsHelper.Snapshot s=PrivateDnsHelper.inspect(this);privateDns.setText((s.isStrict()?"⚠ STRICT • ":"OK • ")+s.pretty()+" • DNS: "+s.dnsServers);privateDns.setTextColor(s.isStrict()?c(R.color.swir_red):c(R.color.swir_green));}
    ...
    private void renderSmart(){
        ...
        if(!service){smartStatus.setText("⚠ Usługa Accessibility wyłączona");smartStatus.setTextColor(c(R.color.swir_red));}
        else if(!detector){smartStatus.setText("Usługa aktywna • detection wyłączone");smartStatus.setTextColor(c(R.color.swir_muted));}
        else{smartStatus.setText(live?"AKTYWNY • analizuje aplikacje":"WŁĄCZONY • oczekiwanie na zdarzenie");smartStatus.setTextColor(live?c(R.color.swir_green):c(R.color.swir_blue));}
        smartStats.setText(p.getLong(SmartAdAccessibilityService.KEY_DETECTIONS,0)+" wykryć • "+p.getLong(SmartAdAccessibilityService.KEY_ACTIONS,0)+" akcji");
    }

    private void renderAi(){if(aiStats==null)return;AdaptiveLearningEngine.Stats s=AdaptiveLearningEngine.stats(this);int community=CommunityLearningManager.count(this);aiStats.setText((AdaptiveLearningEngine.isEnabled(this)?"AI aktywne":"AI wyłączone")+" • feedback="+s.samples+" • model="+s.modelFeatures+" cech • Community="+community);}

    private void renderListInfo(){if(listInfo==null)return;long t=prefs().getLong(BlocklistManager.KEY_LAST_UPDATE,0);listInfo.setText(t<=0?"Wbudowana lista startowa.":"Ostatnia aktualizacja: "+DateFormat.getDateTimeInstance(DateFormat.SHORT,DateFormat.SHORT,Locale.getDefault()).format(new Date(t)));}
    ...
    private void toggleVpn(){
        ...
        if(s.isStrict()){new AlertDialog.Builder(this).setTitle("Konflikt z Prywatnym DNS").setMessage("Masz STRICT Private DNS. Najlepiej ustaw Automatyczny albo Wyłączony.").setNegativeButton("Anuluj",null).setPositiveButton("Ustawienia",(d,w)->openPrivateDns()).setNeutralButton("Uruchom mimo to",(d,w)->prepareVpn()).show();return;}
        prepareVpn();
    }
    ...
    private void startVpnService(){try{Intent i=new Intent(this,AdBlockVpnServiceV121.class).setAction(AdBlockVpnServiceV121.ACTION_START);if(Build.VERSION.SDK_INT>=26)startForegroundService(i);else startService(i);vpnStatus.setText("URUCHAMIANIE…");}catch(Exception e){SystemLogStore.error(this,"VPN","Start failed",e);Toast.makeText(this,"Błąd startu — sprawdź SYSTEM CONSOLE.",Toast.LENGTH_LONG).show();}}

    private void updateLists(){if(updateButton==null||!updateButton.isEnabled())return;updateButton.setEnabled(false);updateButton.setText("POBIERANIE…");new Thread(()->{try{int n=BlocklistManager.updateRemote(getApplicationContext());SystemLogStore.info(this,"BLOCKLIST","Update OK • "+n);runOnUiThread(()->{updateButton.setEnabled(true);updateButton.setText("AKTUALIZUJ LISTY");domainCount.setText(n+" domen na liście");renderListInfo();reloadVpn();});}catch(Throwable t){SystemLogStore.error(this,"BLOCKLIST","Update failed",t);runOnUiThread(()->{updateButton.setEnabled(true);updateButton.setText("AKTUALIZUJ LISTY");Toast.makeText(this,"Błąd list — zobacz konsolę.",Toast.LENGTH_LONG).show();});}},"xADKiller-ListUpdate").start();}
    ...
    private void addRule(boolean block){String d=BlocklistManager.normalize(domainInput.getText().toString());if(d==null){Toast.makeText(this,"Podaj poprawną domenę.",Toast.LENGTH_LONG).show();return;}if(block)BlocklistManager.addCustomBlock(this,d);else BlocklistManager.addAllow(this,d);domainInput.setText("");domainCount.setText(BlocklistManager.currentCount()+" domen na liście");reloadVpn();}

    private void confirmReset(){new AlertDialog.Builder(this).setTitle("Reset cache / ULTRA?").setMessage("Usunie pobraną dużą listę i wyłączy ULTRA. Własne reguły zostaną zachowane.").setNegativeButton("Anuluj",null).setPositiveButton("RESET",(d,w)->{try{startService(new Intent(this,AdBlockVpnServiceV121.class).setAction(AdBlockVpnServiceV121.ACTION_STOP));}catch(Exception ignored){}BlocklistManager.clearRemoteCache(this);renderAll();Toast.makeText(this,"Cache zresetowany.",Toast.LENGTH_LONG).show();}).show();}

    private void loadFullListAsync(){new Thread(()->{try{int n=BlocklistManager.load(getApplicationContext());runOnUiThread(()->{if(domainCount!=null)domainCount.setText(n+" domen na liście");});}catch(Throwable t){SystemLogStore.error(this,"BLOCKLIST","Startup load failed",t);}},"xADKiller-StartupList").start();}
    ...
    private void openAccessibility(){try{startActivity(new Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS));Toast.makeText(this,"Włącz „xADKiller Smart Ad Engine”.",Toast.LENGTH_LONG).show();}catch(Exception e){SystemLogStore.error(this,"SMART","Settings failed",e);}}
    ...
    private TextView tv(String s,float sp,int color,boolean bold){TextView v=new TextView(this);v.setText(s);v.setTextSize(sp);v.setTextColor(color);v.setLineSpacing(0,1.08f);if(bold)v.setTypeface(Typeface.DEFAULT,Typeface.BOLD);return v;}
    private Switch toggle(String text,boolean checked,int color){Switch s=new Switch(this);s.setText(text);s.setTextColor(color);s.setChecked(checked);return s;}
    private Button button(String text,int bg,int fg){Button b=new Button(this);b.setText(text);b.setTextColor(fg);b.setTextSize(12);b.setTypeface(Typeface.DEFAULT,Typeface.BOLD);b.setAllCaps(false);b.setPadding(dp(8),0,dp(8),0);b.setBackground(round(bg,12));return b;}
