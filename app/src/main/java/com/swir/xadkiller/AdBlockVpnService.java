...
public class AdBlockVpnService extends VpnService {
    public static final String ACTION_START = "com.swir.xadkiller.START";
    public static final String ACTION_STOP = "com.swir.xadkiller.STOP";
    public static final String ACTION_RELOAD = "com.swir.xadkiller.RELOAD";
    public static final String ACTION_STATUS = "com.swir.xadkiller.STATUS";
    ...
    @Override public void onCreate() { super.onCreate(); createNotificationChannel(); BlocklistManager.load(this); }

    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        String action = intent != null ? intent.getAction() : null;
        if (ACTION_STOP.equals(action)) { stopVpn(); return START_NOT_STICKY; }
        if (ACTION_RELOAD.equals(action)) { BlocklistManager.load(this); sendStatus(true); return workerRunning.get() ? START_STICKY : START_NOT_STICKY; }
        startInForeground();
        if (workerRunning.compareAndSet(false, true)) { startVpnWorker(); maybeRefreshBlocklist(); }
        return START_STICKY;
    }

    @Override public void onDestroy() { stopVpnInternal(false); super.onDestroy(); }
    ...
    private Notification buildNotification() {
        PendingIntent openPi = PendingIntent.getActivity(this, 1, new Intent(this, MainActivity.class), PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
        PendingIntent stopPi = PendingIntent.getService(this, 2, new Intent(this, AdBlockVpnService.class).setAction(ACTION_STOP), PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
    ...
    private void startVpnWorker() {
        worker = new Thread(() -> {
            try {
                ...
                lastBlocked = "Błąd: " + e.getClass().getSimpleName(); sendStatus(true);
            } finally {
        ...
        try (FileInputStream in = new FileInputStream(pfd.getFileDescriptor()); FileOutputStream out = new FileOutputStream(pfd.getFileDescriptor())) {
            ...
            while (workerRunning.get()) {
                ...
                if (BlocklistManager.isBlocked(q.host)) {
                    ...
                    dnsResponse = forward(q.dnsPayload); if (dnsResponse == null) dnsResponse = DnsPacket.servfail(q.dnsPayload);
                }
                if (dnsResponse != null) {
    ...
    private byte[] forward(byte[] query) {
        for (String server : UPSTREAMS) {
            ...
            try {
                socket = new DatagramSocket(); if (!protect(socket)) continue; socket.setSoTimeout(2200);
                ...
                if (query.length >= 2 && (buf[0] != query[0] || buf[1] != query[1])) continue;
                return Arrays.copyOf(buf, response.getLength());
            } catch (SocketTimeoutException ignored) {} catch (Exception ignored) {} finally { if (socket != null) socket.close(); }
        }
        return null;
    }
    ...
    private void maybeRefreshBlocklist() {
        ...
        new Thread(() -> { try { BlocklistManager.updateRemote(getApplicationContext()); sendStatus(true); } catch (Exception ignored) {} }, "xADKiller-AutoList").start();
    }
